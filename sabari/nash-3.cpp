...
* Last Modified : Wednesday 01 January 2014 05:39:07 PM IST
...
int main() {
    ...
    while(t--) {
        ...
        REP(i, n) {
            ll maxi = pay_off[1][0][i];
            REP(j, m) maxi = max(pay_off[1][j][i], maxi);
            REP(j, m) best_response[1][j][i] = (maxi == pay_off[1][j][i]);
        }
